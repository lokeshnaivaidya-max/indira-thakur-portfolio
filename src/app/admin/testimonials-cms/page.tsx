...
export default function AdminTestimonialsPage() {
  ...
  const addTestimonial = () => {
    const items = [...(t.testimonials || []), { quote: '', author: '', role: '', avatar: { url: '', alt: '' } }];
    updateSection('testimonials', { testimonials: items });
  };
      ...
      <div className="flex-1 overflow-y-auto space-y-6 max-w-4xl mx-auto w-full">
        <Section title="Section Header" defaultOpen>
          <div className="grid grid-cols-2 gap-4">
            <FieldInput label="Eyebrow" value={t.eyebrow || ''} onChange={(v) => updateSection('testimonials', { eyebrow: v })} placeholder="e.g., Kind Words" />
            <FieldInput label="Heading" value={t.heading || ''} onChange={(v) => updateSection('testimonials', { heading: v })} placeholder="e.g., What Families Say" />
          </div>
          <ImageManager
        ...
        <Section title={`Testimonials (${(t.testimonials || []).length})`} defaultOpen>
            ...
            {(t.testimonials || []).map((item: any, i: number) => (
              <div key={i} className="p-5 bg-ivory/50 border border-cream/40 rounded-lg space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <FieldInput label="Author Name" value={item.author || ''} onChange={(v) => handleTestimonialChange(i, 'author', v)} placeholder="e.g., Ananya Sharma" />
                  <FieldInput label="Role (optional)" value={item.role || ''} onChange={(v) => handleTestimonialChange(i, 'role', v)} placeholder="e.g., New Mom" />
                </div>
                <ImageManager
