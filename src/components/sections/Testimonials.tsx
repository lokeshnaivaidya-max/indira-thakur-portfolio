            ...
            >
              ...
              {items[current]?.rating && items[current].rating > 0 && (
                <div className="flex items-center justify-center gap-0.5 mt-3">
                  {[1,2,3,4,5].map((star) => (
                    <svg key={star} className={`w-3.5 h-3.5 ${(items[current].rating || 0) >= star ? 'text-magenta fill-current' : 'text-warm-gray/40'}`} stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
