      ...
      <div className="container-editorial py-20 md:py-28 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-16">
          ...
          <nav aria-label="Footer navigation">
            ...
            <ul className="space-y-3">
              {[
                ...
                <li key={item.href}>
                  <Link href={item.href} className="font-sans text-sm text-white/40 hover:text-[#B76E79] transition-colors duration-500 py-1.5 min-h-[44px] inline-block">
                    {item.label}
                  </Link>
                </li>
              ))}
          ...
          <nav aria-label="Footer navigation">
            ...
            <ul className="space-y-3">
              <li><a href={`mailto:${footerData.email}`} className="font-sans text-sm text-white/40 hover:text-[#B76E79] transition-colors duration-500 py-1.5 min-h-[44px] inline-block">{footerData.email}</a></li>
              <li><a href={`tel:${footerData.phone?.replace(/\s/g, '')}`} className="font-sans text-sm text-white/40 hover:text-[#B76E79] transition-colors duration-500 py-1.5 min-h-[44px] inline-block">{footerData.phone}</a></li>
              {footerData.instagramUrl && (
                <li><a href={footerData.instagramUrl} target="_blank" rel="noopener noreferrer" className="font-sans text-sm text-white/40 hover:text-[#B76E79] transition-colors duration-500 py-1.5 min-h-[44px] inline-block">Instagram</a></li>
              )}
              {footerData.facebookUrl && (
                <li><a href={footerData.facebookUrl} target="_blank" rel="noopener noreferrer" className="font-sans text-sm text-white/40 hover:text-[#B76E79] transition-colors duration-500 py-1.5 min-h-[44px] inline-block">Facebook</a></li>
              )}
            </ul>
          </nav>
        </div>
