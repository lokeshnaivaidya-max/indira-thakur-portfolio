...
export async function uploadToCloudinaryDirect(
  ...
  // Validate file type
  const allowedTypesList = ['image/jpeg', 'image/png', 'image/webp', 'image/gif'];
  if (!allowedTypesList.includes(file.type)) {
    throw new Error(`File type ${file.type} is not allowed. Allowed types: ${allowedTypesList.join(', ')}`);
  }
  ...
  formData.append('folder', 'indira-thakur/gallery');
...
export function useDirectUpload() {
  ...
  const upload = useCallback(async (file: File) => {
    ...
    try {
      ...
      const result = await new Promise<{ url: string; publicId: string; width: number; height: number }>((resolve, reject) => {
        const xhr = new XMLHttpRequest();
  ...
  const cancel = useCallback(() => {
    // Note: XMLHttpRequest doesn't easily support cancellation mid-upload
    // This would require storing the xhr reference
  }, []);
